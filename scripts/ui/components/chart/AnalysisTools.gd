...
func _draw_spread_hover_tooltip(spread: float, margin_pct: float):
	...
	if station_data.has("profit_margin") and station_data.has("skill_benefits"):
		...
		lines = [
			...
			"Broker Relations Lv%d:" % int(broker_level),
			"  • Fee: %.1f%% (saved %.1f%%)" % [float(current_broker_fee), float(broker_savings)],
			"  • Default: 2.5% → Your Rate: %.1f%%" % float(current_broker_fee),
			"",
			"Accounting Lv%d:" % int(accounting_level),
			"  • Tax: %.1f%% (saved %.1f%%)" % [float(current_sales_tax), float(sales_tax_savings)],
			"  • Default: 8.0% → Your Rate: %.1f%%" % float(current_sales_tax),
			"",
